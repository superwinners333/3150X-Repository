...
void Macro(void)
{
  ...
  while(MacroActiv==1) {
    ...
    if(abs(LiftSensor.position(degrees)) < 23.5) { // 32
      RunArms(40);
      if(abs(LiftSensor.position(degrees)) > 18.5) { // 25
        MacroActiv = 0;
        StopArms();
      }
    } 
    else if(abs(LiftSensor.position(degrees)) > 23.5) { // 25
      RunArms(-40);
      if(abs(LiftSensor.position(degrees)) <  18.5) { // 32
        MacroActiv = 0;
        StopArms();
      }
    } 
