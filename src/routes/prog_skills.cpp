...
void prog_skills() 
{
    ...
    wait(400,msec);
    MoveEncoderPID(TestPara, -100, -9.5, 0.2, 0,true); // drives away from wall
    wait(50,msec);
    ...
    MoveEncoderPID(TestPara, -100, -28.7, 0.3, 39.3,true); // drives to rings in front of wall stake
    wait(20,msec);
    ...
    RunArms(0);
    wait(20,msec);
    MoveEncoderPID(TestPara, 100, -6.3, 0.4, 90,false); // moves back
    RunArms(-100);
    wait(500,msec);
    RunArms(0);
    ...
    wait(50,msec);
    MoveEncoderPID(TestPara, -80, -45, 0.6, 180,false); // picks up the 3 rings that are in a line
    ...
    Clamp.set(false);
    wait(250,msec);
    MoveEncoderPID(TestPara, -100, -5.2, 0.3, -40,true); // drives away from corner
    wait(70,msec);
    ...
    wait(50,msec);
    MoveEncoderPID(TestPara, 100, -101.3, 0.6, 90, false); // drives to mogo
    MoveEncoderPID(TestPara, 30, -34.5, 0.2,90,false); // slows down before grabbing mogo
    wait(30,msec);
    Clamp.set(true);
    wait(100,msec);
    // MoveEncoderPID(TestPara, -60, 1.5, 0.2,90,true);
    TurnMaxTimePID(TestPara, 0, 0.4, true); // turns to ring between bot and ladder
    RunRoller(100);
    wait(20,msec);
    MoveEncoderPID(TestPara, -100, -27.5, 0.2, 0,true); // drives to the ring between bot and ladder

    // OTHER SIDE WALL STAKES
    wait(20,msec);
    TurnMaxTimePID(TestPara, -36, 0.4, false); // turns to in front of wall stake
    MoveEncoderPID(TestPara, -100, -30.6, 0.2, -36, true); // drives to in front of wall stake
    wait(40,msec);
    Macro(); // activates macro
    TurnMaxTimePID(TestPara, -90, 0.4, false); // turns towards wall stake
    ...
    Move(0,0);
    RunArms(0);
    MoveEncoderPID(TestPara, 100, -12.7, 0.4, -90,true); // moves back
    ...
    MoveEncoderPID(TestPara, -50, -40, 0.5, 180,false); 
    wait(100,msec);
    ...
    Clamp.set(false);
    wait(50,msec);
    MoveEncoderPID(TestPara, -100, 11.5, 0.5, 40,false); // backs away from corner
    wait(50,msec);
    ...
    MoveEncoderPID(TestPara, -100, 96.5, 0.4, 0, true); // drives to past the white line
    wait(80,msec);
    RunRoller(60);
    TurnMaxTimePID(TestPara, 90, 0.4, true); // turns to next ring
    MoveEncoderPID(TestPara, -100, 32.8, 0.4, 90, true); // drives to it
    wait(40,msec);
    // Gyro.setHeading(0,degrees);
    
    // PUSHING MOGO INTO CORNER AND CLAMPING ANOTHER ONE
    TurnMaxTimePID(TestPara, -3, 0.4, true); // turns to face mogo
    RunRoller(0);
    MoveEncoderPID(TestPara, -100, -34, 0.2, -3, false); // drives into mogo
    
    TurnMaxTimePID(TestPara, -80, 0.3, false);
    MoveTimePID(TestPara, 100, 1.25, 0.4, -80, false);
    ...
    MoveEncoderPID(TestPara, 100, -53, 0.4, -72, false); // drives backwards into next mogo
    MoveEncoderPID(TestPara, 30, -42, 0.4, -72, false); // slows down
    wait(50,msec);
    Clamp.set(true);
    wait(80,msec);

    // SCORES FINAL 
    TurnMaxTimePID(TestPara, 155, 0.5, false); // turns to next ring
    RunRoller(100);
    wait(30,msec);
    MoveEncoderPID(TestPara, -100, -21, 0.4, 155, true); // grabs ring
    TurnMaxTimePID(TestPara, 90, 0.4, true); // turns to next ring
    MoveEncoderPID(TestPara, -100, -27.3, 0.4, 90, true); // grabs ring

    TurnMaxTimePID(TestPara, 0, 0.5, false); // turns to the 2 rings in a row
    MoveEncoderPID(TestPara, -80, -21, 0.6, 0, false); // grabs both of the rings
    MoveEncoderPID(TestPara, -50, -21, 0.6, 0, false);
    wait(80,msec);
    MoveEncoderPID(TestPara, 100, -5.5, 0.4, 0, false); // backs up
    wait(20,msec);
    ...
    // Doinker.set(true);
    wait(250,msec);
    // TurnMaxTimePID(TestPara, -40, 0.2, false); // turns so mogo faces corner
    TurnMaxTimePID(TestPara, -140, 0.2, true); // turns so mogo faces corner
    // Doinker.set(false);
    wait(300,msec);

    // PUTTING OTHER MOGO IN CORNER
    
    MoveTimePID(TestPara, -50, 1, 0.4, -140, true); // backs up into corner
    wait(100,msec);
    Clamp.set(false);
    RunRoller(0);
    wait(100,msec);
    ...
    wait(50,msec);
    TurnMaxTimePID(TestPara, 70, 0.4, false); // Turns to face ladder
    RunArms(100);
    MoveEncoderPID(TestPara, 100, -9, 0.3, 70,false);
    wait(170,msec);
    // Doinker.set(false);
    StopArms();
    MoveEncoderPID(TestPara, 100, -67, 0.4, 55,false);
    MoveEncoderPID(TestPara, 90, -20, 0.4, 55,false);
    RunArms(-100);
    wait(500,msec);
    RunArms(0);
