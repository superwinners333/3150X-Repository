...
void prog_skills() 
{
    ...
    RunArms(-100);
    wait(20,msec);
    MoveEncoderPID(TestPara, 80, -4.2, 0.4, 90,true); // moves back
    wait(430,msec);
    RunArms(0);
    MoveEncoderPID(TestPara, 80, -4.2, 0.5, 90,true); // moves back
    ...
    Clamp.set(false);
    wait(100,msec);
    MoveEncoderPID(TestPara, -70, -5.5, 0.5, -32.5,true); // drives away from corner
    ...
    MoveEncoderPID(TestPara, -70, -84, 0.5, 180,false); // picks up the 3 rings that are in a line
    wait(90,msec);
    MoveEncoderPID(TestPara, 80, -18, 0.4, 180,false); // backs up
    wait(70,msec);
    TurnMaxTimePID(TestPara, -140, 0.5, false); // turns to side ring
    ...
    RunRoller(100);
    MoveEncoderPID(TestPara, -100, -55, 0.4, 45, true); // grabs next ring
    RunRoller(0);
