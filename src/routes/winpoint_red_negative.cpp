...
void winpoint_red_negative() 
{
    ...
    wait(250,msec);
    RunRoller(0); // stop intake
    MoveEncoderPID(TestPara, 100, -34 , 0.4,0,true); // move to mogo
    wait(250,msec);
    Clamp.set(true); // clamp on mogo
    wait(250,msec);
    MoveEncoderPID(TestPara, 100, -9 , 0.4,0,true); // move back 
    wait(250,msec);
    TurnMaxTimePID(TestPara, 135, 0.9, true); // rotate to face ring
    RunRoller(-100); // run intake to prepare for intaking ring
    wait(250,msec);
    MoveEncoderPID(TestPara, 100, 24 , 0.4,0,true); // move forward to ring
