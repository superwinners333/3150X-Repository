...
{   
    ...
    TurnMaxTimePID(TestPara, -36, 0.4, false); // turns to mogo
    MoveEncoderPID(TestPara,100,21,0.2,-36,true); // drives to mogo
    MoveEncoderPID(TestPara,30,21,0.2,-42,true);
    Clamp.set(true);
    wait(40,msec);
    RunRoller(100);
    TurnMaxTimePID(TestPara, 90, 0.3, false); // turns to last ring
    MoveEncoderPID(TestPara,-100,38,0.2,90,false); // grabs ring
